...
class PreprocessWords():
    def __init__(self):
        ...
        self.abbrs = {"don't": "do not", "Don't": "do not", "doesn't": "does not", "Doesn't": "does not",
                      ...
                      "it's": "it is", "It's": "it is", "won't": "will not", "wo n't": "will not", " ;ll": " will",
    ...
    def pre_words(self, sent, api_names, useless_NN, result):
        ...
        '''time'''
        sent = re.sub('note that|Note that|#|that is', ' ', sent)
        sent = re.sub(r'\d+:\d+:\d+', 'time', sent)

        '''replace less seen words'''
        sent = re.sub('obsoleted', 'deprecated', sent)
        sent = re.sub('prior to', 'before', sent)
        sent = re.sub(r' is up to | are up to | was up to | were up to ', ' be upto ', sent)
        sent = re.sub(r'if available\w*|If available\w*', '', sent)
        sent = re.sub('regardless of[\w ]+', '', sent)
        ...
        sent = re.sub(r'type \w+_\S+', 'constant', sent)

        '''replace AA_BB with constant, NEEDED'''
        sent = re.sub(r'([A-Z]+\d*_)+[A-Z\d\.]+', 'constant', sent)
        sent = re.sub(r'\S+_constant|\d+_APIConstant', 'constant', sent)
        ...
        sent = sent.replace('-', '_')
        sent = re.sub(r'\w+_\S+', 'param', sent)  # TODO: var? new change to constant
        ...
        sent = sent.replace(' _ |^_ | _$', ' ') # ' _a' will become ' _ a' after spacy manipulation, so we remove ' _ '
        ...
        ''' skip too short sentence '''
        if len(re.sub(r'[^ \w]', '', sent).split()) < 2:
            return
